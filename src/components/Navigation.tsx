...
export function Navigation() {
  ...
  return (
    <>
      <header className="w-full relative z-40 py-4 md:py-6 px-4 md:px-8 lg:px-12 flex items-center justify-between bg-surface/80 backdrop-blur-md md:bg-transparent md:backdrop-blur-none transition-all duration-300">
        <Link
          href="/"
          className="text-3xl md:text-4xl text-primary font-extrabold tracking-tighter"
          ...
          {links.map((link) => {
            ...
            return (
              ...
              >
                {link.label}
                {isActive && (
                  <div className="absolute -bottom-2 left-0 right-0 h-1 bg-primary-container opacity-80 decoration-wavy rounded-full transform -rotate-1" />
                )}
              </Link>
            );
          })}
        ...
        <div className="flex items-center gap-4">
          ...
          {!isOpen && (
            <button 
              className="md:hidden p-2 text-primary rounded-full bg-white shadow-sm border border-primary/10"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 bg-surface md:hidden flex flex-col pt-24 px-6"
          ...
          >
            ...
            <div className="w-full flex justify-end p-4 md:p-6 relative z-20">
              <button 
                className="p-2 text-primary rounded-full bg-white shadow-md border border-primary/20"
            ...
            <nav className="flex flex-col gap-8 relative z-10 items-center text-center mt-4 overflow-y-auto pb-12 flex-1">
              ...
              <div className="w-full max-w-xs h-0.5 bg-primary/20 my-4" />
              ...
              <div style={{ fontFamily: "var(--font-caveat)" }} className="mt-8 text-center text-on-surface/60 text-3xl rotate-2 px-6">
                "The Chaos continues..."
