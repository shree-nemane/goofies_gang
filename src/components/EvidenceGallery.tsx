      ...
      {items.map((img, i) => (
        ...
        >
          <div className="bg-white p-3 md:p-4 shadow-ambient">
            ...
            <div className="w-full bg-gray-200 mb-3 md:mb-4">
              <img 
                src={`/api/evidence/${img.id}/image`} 
                alt={`Evidence photo: ${img.caption} - uploaded on ${formatDate(img.createdAt)}`} 
                loading="lazy"
                decoding="async"
                className="w-full h-auto grayscale-[0.3] group-hover:grayscale-0 transition-all duration-500 rounded-sm" 
              />
            </div>
