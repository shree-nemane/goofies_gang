...
export function DeleteButton({ id, type, className }: { id: string, type: "quote" | "evidence" | "roast", className?: string }) {
  ...
  const renderModal = () => {
    ...
    return createPortal(
      <AnimatePresence>
        {modal !== "closed" && (
          <div className="fixed inset-0 flex items-center justify-center z-[9999] bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
            {modal === "confirm" && (
              <motion.div 
                initial={{ opacity: 0, scale: 0.9, rotate: -1 }}
                animate={{ opacity: 1, scale: 1, rotate: 0 }}
                exit={{ opacity: 0, scale: 0.9, rotate: -1 }}
                className="bg-[#fdf6e3] p-8 md:p-10 border-4 border-[#322f22] shadow-[8px_8px_0px_#ff728d] relative max-w-sm w-full z-10 my-auto"
              >
            ...
            {modal === "password" && (
              <motion.div 
                ...
                animate={{ opacity: 1, scale: 1, rotate: 0 }}
                exit={{ opacity: 0, scale: 0.9, rotate: 1 }}
                className="bg-[#fdf6e3] p-8 md:p-10 border-4 border-[#322f22] shadow-[8px_8px_0px_#a8e6cf] relative max-w-sm w-full z-10 my-auto"
              >
                {/* Sticky Tape at the top */}
